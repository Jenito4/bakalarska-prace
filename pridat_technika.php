...
<div class="row">
      <div class="col-sm-1">
        ...
        <form enctype="multipart/form-data" action="php/pridat_technika_skript.php" method="post">
      ...
      <div class="col-sm-5">
        <table class="table">
          ...
          <tr class="bolder">
          <label for="prirad">Technik:* </label><br>

                 <select class="form-control" name="prirad" id="prirad" placeholder="Přiřaďte technika" required>
                   <?php
                     $sql = "SELECT id_technik, jmenoTechnik, prijmeniTechnik FROM technici";

                     if($query = mysqli_query($conn, $sql)){
                       while($row = mysqli_fetch_array($query)){
                         if($row["id_technik"] == $id_technik){
                           echo "<option value=".$row["id_technik"]." selected>".$row["jmenoTechnik"]." ".$row["prijmeniTechnik"]."</option>\n";
                         } else {
                           echo "<option value=".$row["id_technik"].">".$row["jmenoTechnik"]." ".$row["prijmeniTechnik"]."</option>\n";
                         }
             ...
             <input class="btn btn-primary" type="submit" value="Přidat technika" /><br /><br />
        &emsp;
        &emsp;

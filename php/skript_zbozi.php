...
if(!$error && !empty($_FILES['ZboziPhoto']['name'])){
  if(file_exists("../../foto_zbozi/".$_FILES['ZboziPhoto']['name'])){
      $length = (int)"20";
      ...
      $newName = $randomString."".$_FILES['ZboziPhoto']['name'];
      $_FILES['ZboziPhoto']['name'] = $newName;
  }
...
if(!$error){
  ...
  if(!empty($_FILES['ZboziPhoto']['name'])){
    $photoName = $_FILES['ZboziPhoto']['name'];
  }
...
function addIfPhoto(){
  $valid_formats = array("jpg", "png", "bmp", "PNG", "JPG", "jpeg", "JPEG");
   ...
   if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST"){
    $name = $_FILES['ZboziPhoto']['name'];
     if ($_FILES['ZboziPhoto']['error'] == 4) {
         continue;
     }
     if ($_FILES['ZboziPhoto']['error'] == 0) {
         if ($_FILES['ZboziPhoto']['size'] > $max_file_size) {
             // fotka pøíliš velká
             continue;
         }
     elseif( ! in_array(pathinfo($name, PATHINFO_EXTENSION), $valid_formats) ){
       ...
       continue;
             ...
             if(move_uploaded_file($_FILES["ZboziPhoto"]["tmp_name"], $path.$name))
